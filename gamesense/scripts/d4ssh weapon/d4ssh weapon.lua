...
local master_switch = ui_new_checkbox("LUA", "A", "Enable d4ssh adaptive wpn")
...
for i=1, #config_names do
    rage[i] = {
        ...
        quick_stop_options = ui_new_multiselect("LUA", "A", "[" .. config_names[i] .. "] Quick stop options", {"Early", "Slow motion", "Duck", "Fake duck", "Move between shots", "Ignore molotov", "Taser", "Jump scout"}),
        
        quick_stop_ns = ui_new_checkbox("LUA", "A", "[" .. config_names[i] .. "] Quick stop noscope"),
        quick_stop_options_ns = ui_new_multiselect("LUA", "A", "[" .. config_names[i] .. "] Quick stop options ns", {"Early", "Slow motion", "Duck", "Fake duck", "Move between shots", "Ignore molotov", "Taser", "Jump scout"}),
...
rage[scoped_wpn_idx[1]].js_multimode = ui_new_combobox("LUA", "A", "\n[Jump Scout] Multi-point mode", { "Low", "Medium", "High" })
...
rage[scoped_wpn_idx[1]].js_quick_stop_options = ui_new_multiselect("LUA", "A", "[Jump Scout] Quick stop options", {"Early", "Slow motion", "Duck", "Fake duck", "Move between shots", "Ignore molotov", "Taser", "Jump scout"})
...
rage[scoped_wpn_idx[1]].js_force_baim_peek = ui_new_checkbox("LUA", "A", "[Jump Scout] Force body aim on peek")
...
local ref_doubletap, ref_doubletapkey, ref_doubletap_mode = ui_reference("RAGE", "Aimbot", "Double tap")
...
local function handle_menu()
    ...
    for i=1, #config_names do
        ...
        ui_set_visible(rage[i].dt_multimode, show and ui_get(rage[i].dt_mp_enable) and #{unpack(ui_get(rage[i].multipoint))} > 0)
    ...
    ui_set_visible(rage[scoped_wpn_idx[1]].js_force_baim_peek, js_enable)   
